...
using HarmonySound.MVC.Infrastructure;
...
namespace HarmonySound.MVC
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            // Persistencia de las claves de Data Protection en PostgreSQL.
            // Así las cookies de sesión/login y los tokens antiforgery sobreviven a los redeploys
            // de Render (que descarta el sistema de archivos del contenedor en cada despliegue).
            var keysConnectionString = builder.Configuration.GetConnectionString("HarmonySoundDbContext")
                ?? Environment.GetEnvironmentVariable("ConnectionStrings__HarmonySoundDbContext");

            if (!string.IsNullOrWhiteSpace(keysConnectionString))
            {
                builder.Services.AddDbContext<DataProtectionKeysContext>(options =>
                    options.UseNpgsql(keysConnectionString));

                builder.Services.AddDataProtection()
                    .SetApplicationName("HarmonySound") // Mismo nombre entre despliegues para descifrar tokens antiguos
                    .PersistKeysToDbContext<DataProtectionKeysContext>();
            }
            else
            {
                Log.Warning("No se configuró la cadena de conexión 'HarmonySoundDbContext'; las claves de Data Protection NO se persistirán y las sesiones se invalidarán en cada reinicio.");
            }

            var app = builder.Build();

            // Asegura que la tabla de claves exista, sin tocar el resto del esquema (gestionado por la API).
            if (!string.IsNullOrWhiteSpace(keysConnectionString))
            {
                ...
                {
                    Log.Error(ex, "No se pudo asegurar la tabla DataProtectionKeys en la base de datos.");
                }
            }
